...
float CalcDirectionalShadowFactor(DirectionalLight light){
	...
	for(int x = -1; x <= 1; ++x){
		for(int y = -1; y <= 1; ++y){
			...
			shadowed += (current - bias) > pcfDepth ? 1.0 : 0.0;
		}
	}
...
float CalcOmniShadowFactor(PointLight light, int shadowIndex){
	...
	closest = closest * omniShadowMaps[shadowIndex].farPlane;
...
vec4 CalcLightByDirection(Light light, vec3 direction, float shadowFactor){
	...
	if(diffuseFactor > 0.0f) {
		...
		if(specularFactor > 0.0) {
			...
			specularColour = vec4(light.color * material.specularInstensity * specularFactor, 1.0);
		}
	}

	return (ambientColor + (1.0 - shadowFactor) * (diffuseColor + specularColour));
}
...
vec4 CalcSpotLight(SpotLight sLight, int shadowIndex){
	...
	if(slFactor > sLight.edge) {
		...
		colour = colour * (1.0 - (1.0 - slFactor)*(1.0 / (1.0 - sLight.edge)));
